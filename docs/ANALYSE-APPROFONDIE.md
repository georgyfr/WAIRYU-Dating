...
> **Mise à jour post-sondes (23/09/2026)** — compte réel testé : plan Free 25 GB confirmé, assets `authenticated` + signatures serveur **validées en conditions réelles**. Une limite a été découverte : l'expiration des URLs via `v=<timestamp>` **n'est pas appliquée** par Cloudinary. Le modèle de sécurité reste intact (issue d'URL contrôlée par le Worker, public_ids non énumérables, transformations signées) mais la garantie « URL qui meurt après 15 min » est remplacée par « URL délivrée uniquement après autorisation ». Détails, algorithmes et limites : [STOCKAGE-CLOUDINARY.md](STOCKAGE-CLOUDINARY.md).
